...
int main(){ 
	...
	float posit_smearingFactor = 100.0; // Example smearing factor for position
...
Pythia8::Vec4 PositionResSmear(Pythia8::Vec4 photon, double smearingFactor) {//, Pythia8::Rndm* rndm
	...
	std::cout << "old Z:New Z = " << photon.pz()*photon.pz() <<" : " << znew*znew << " , New Phi = " << atan(photon.py()/photon.px())*180/3.1415 <<" : " << atan(ynew/xnew)*180/3.1415 << std::endl;
