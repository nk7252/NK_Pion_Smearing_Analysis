...
int main(){ 
	...
	float smeared_upper_bin_limit = 0.5;
	...
	for (int smear_factor_itt = 0; smear_factor_itt < 0 + 1; smear_factor_itt++){// originally int smear_factor_itt = 0; smear_factor_itt < 24 + 1; smear_factor_itt++
		...
		TFile *output = new TFile(Form("pioncode/rootfiles/PionFastMC_%f_sqrte_%f_const.root", smear_factor_b , smear_factor_c ), "recreate");//
		...
		//pythia.readString("111:all = pion -> gamma gamma");
		pythia.readString("111:oneChannel = 1 1.0 0 22 22");// force pion->gammagamma with 100% branching ratio?
		pythia.init(); 
		pythia.event.clear();
		...
		for (int i = 0; i < NPions; i++){//create primaries
			...
			pi0_E = sqrt(Pion_M * Pion_M + Pt * Pt + pi0_pz * pi0_pz);
             // 111 is pi0 add a partice, 111 is pi0
			if (pythia.event.append(111, 23, 0, 0, pi0_px, pi0_py, pi0_pz, pi0_E, Pion_M)) {
            pi0Count++;
        	} else {
            	std::cout << "Failed to append pi0 meson at iteration " << i << std::endl;
        	}
